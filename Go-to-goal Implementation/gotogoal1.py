...
def orient():
    print("aaya")
    global angle,subimu,terminate1,get,A,count,angle1
    if(get == 1):
        travel()
        get = 2
        print(f"get:{get}")
    ...
    print("count = ",count)
    print(angle1)
    if((angle1<3.14 and angle1>0) or (angle1<-3.14 and angle1>-6.28)):
        print(angle)
        if(angle> 0.07 and angle <= 3.14):
            send.data[0] = 145        #clockwise 
            send.data[1] = 0.0 
        ...
        print(angle)
        if(angle >3.14 and angle<= 6.28):
            send.data[0] = 145      #anticlockwise 
            send.data[1] = 1.0
